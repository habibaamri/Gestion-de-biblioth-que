...
<body>
    ...
    <div class="modal-content">
					      <h4 style="float:center;">Ajouter un nouveau guide de voyage</h4>
	 <form id="Form" method="POST" action="" class="collection" style="padding:20px; " >
				  <div class="row">
				           <div class="input-field col s3">
				            	 <input type="text" class="validate"  value="" name="nom" >
				                <label for="username" class="active">Nom</label>
				            </div>
				              <div class="input-field col s3">
           	                         <input type="text" class="validate" name="des" value="">
				                    <label for="username" class="active">Description</label>
				              </div>
				     </div>         
				    <div class="row">
				            <div class="input-field col s3">
			                    <input type="text" class="validate" name="nombre" value="" >
				                <label for="username" class="active">Nombre d'exemplaire</label>
				            </div>
	...
	<c:if test="${ empty param.nom or empty param.des  or empty param.nombre or empty param.photo}">
     	<c:redirect url="addguide.jsp" >
              <c:param name="InscMsg" value="SVP veeuillez remplir tous les champs !!" />
      </c:redirect>
  	</c:if>
  	<c:if test="${not empty param.nom and not empty param.des and not empty param.nombre and not empty param.photo}">
	...
	INSERT INTO guide_voyage(Nom,Description,Preface,NbrExemplaire,Disponibilité) VALUES (?,?,?,?,?)
	...
	<c:if test="${updatedTable>=1}">
	   <c:redirect url="guidevoyage.jsp"></c:redirect>
	</c:if>
		</c:if>
